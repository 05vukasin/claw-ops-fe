...
interface MicrosoftNodeProps {
  account: MicrosoftAccountWithUI;
  serverX: number;
  serverY: number;
...
export const MicrosoftNode = memo(function MicrosoftNode({ account, serverX, serverY, onMoveEnd, onSpringPos, onSelect, zoom }: MicrosoftNodeProps) {
  ...
  return (
    ...
    >
      ...
      <p className="mt-1 text-center text-[9px] leading-tight text-canvas-muted" style={{ maxWidth: NODE_SIZE + 16 }}>
        <span className="block truncate">{account.accountEmail ?? "Microsoft"}</span>
      </p>
    </div>
  );
});
