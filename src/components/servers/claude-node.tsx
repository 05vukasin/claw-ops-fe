...
/* ------------------------------------------------------------------ */
/*  Claude sparkle icon (inline SVG)                                   */
/* ------------------------------------------------------------------ */
...
export function ClaudeNode({ account, serverX, serverY, onMoveEnd, onSpringPos, onSelect, zoom }: ClaudeNodeProps) {
  ...
  function startSpring() {
    ...
    function tick(time: number) {
      ...
      s.x += s.vx * dt;
      s.y += s.vy * dt;
      const settled = Math.abs(s.vx) < SETTLE_V && Math.abs(s.vy) < SETTLE_V && Math.abs(dx) < SETTLE_D && Math.abs(dy) < SETTLE_D;
      if (settled) { s.x = t.x; s.y = t.y; s.vx = 0; s.vy = 0; applyPosition(s.x, s.y); rafId.current = 0; return; }
  ...
  const handlePointerDown = useCallback((e: React.PointerEvent) => {
    ...
    spring.current.vx = 0; spring.current.vy = 0;
  }, [offset]);
  ...
  const handlePointerMove = useCallback((e: React.PointerEvent) => {
    ...
    spring.current.x = sx; spring.current.y = sy;
  ...
  return (
    ...
    >
      ...
      <p className="mt-1 text-center text-[9px] leading-tight text-canvas-muted" style={{ maxWidth: NODE_SIZE + 16 }}>
        <span className="block truncate">{account.version ?? "Claude"}</span>
      </p>
    </div>
  );
}
