...
// Spring physics — identical to agent-node
const STIFFNESS = 220;
const DAMPING = 18;
const SETTLE_V = 0.05;
const SETTLE_D = 0.15;
...
export function GitHubNode({ account, serverX, serverY, onMoveEnd, onSpringPos, onSelect, zoom }: GitHubNodeProps) {
  ...
  // Spring state — mutable for performance
  ...
  // Initial position from spring state (same pattern as agent-node)
  ...
  return (
    ...
    >
      <div
        ...
        } bg-[#0d1117] dark:bg-[#e8e8e8]`}
        style={{ width: NODE_SIZE, height: NODE_SIZE }}
      >
        <FiGithub size={22} className="pointer-events-none text-white dark:text-[#0d1117]" />
        {/* Status dot */}
      ...
      <p className="mt-1 text-center text-[9px] leading-tight text-canvas-muted" style={{ maxWidth: NODE_SIZE + 16 }}>
        <span className="block truncate">{account.username ?? "GitHub"}</span>
      </p>
    </div>
  );
}
